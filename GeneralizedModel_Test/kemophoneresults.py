...
def datasets_metrics():
    ...
    for dataset in ["KEmoPhone_47_fold"]:
...
def count_test_classes_representation():
    ...
    for dataset in ["KEmoPhone"]:
        ...
        for folder_name in folder_names:
            if folder_name.startswith("KEmoPhone_47_"):
    ...
    results.append(["KEmoPhone", np.mean(y_num), np.std(y_num)])
...
def print_classification_metrics_for_LOSO(results):
    ...
    with pd.option_context("display.float_format", "{:,.2f}".format):
        latex = prepare_latex_for_paper(best_results.to_latex(index=False, column_format="|l|l|r|r|r|"),
                                        f"Best performing model for KEmoPhone in detail", f"tab:datasetsClassesLOSO")
...
if __name__ == '__main__':
    ...
    create_file_for_LOSO(results, "KEmoPhone")
    ...
    create_file_for_cd_diagram(results, "KEmoPhone")
